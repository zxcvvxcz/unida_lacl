...
from models.adspt import SoftEmbedding, RoBERTa_AdSPT_no_verb, BERT_AdSPT_no_verb
...
def main(args, save_config):
    ...
    if 'source_domain' in args.dataset:
        ...
        log_dir = f'{args.log.output_dir}/{args.dataset.name}/adspt_no_verb/{source_domain}-{target_domain}/common-class-{args.dataset.num_common_class}/{args.train.seed}/{args.train.plm_lr}_{args.train.domain_lr}/{args.train.lr}'
    # clinc, massive, trec
    else:
        source_domain = None
        target_domain = None
        ...
        ## LOGGINGS ##
        log_dir = f'{args.log.output_dir}/{args.dataset.name}/adspt_no_verb/{split}/common-class-{args.dataset.num_common_class}/{args.train.seed}/{args.train.plm_lr}_{args.train.domain_lr}/{args.train.lr}'
    print(f'{log_dir=}')
    # init logger
    ...
    ## GET DATALOADER ##
    '''
    load data with soft prompt
    [CLS] text [SOFT]*n_tokens [MASK] [SEP]
    to change prompt, change preprocess_function_prompt_soft in get_dataloaders_prompt
    '''
    ...
    '''
    If BERT, change to BERT_AdSPT_no_verb
    '''
    model = RoBERTa_AdSPT_no_verb(
    ...
    '''
    different learning rate for PLM + embedding / classifier / discriminator
    '''
    mlm_params = {"params": [p for n, p in model.named_parameters() if n.startswith("classifier")], 'lr':args.train.lr}
    disc_params = {"params": [p for n, p in model.named_parameters() if n.startswith("discriminator")], 'lr':args.train.domain_lr}
    plm_params = {"params": [p for n, p in model.named_parameters() if not (n.startswith("classifier") or n.startswith("discriminator"))], 'lr':args.train.plm_lr}
    # AdSPT paper used Adam optimizer.
    all_optimizer = optim.AdamW([mlm_params, disc_params, plm_params])
        ...
        for current_epoch in range(1, args.train.num_train_epochs+1):
            ...
            with tqdm(range(num_step_per_epoch)) as pbar:
                for current_step in pbar:
                    ...
                    '''
                    Simple mean of discriminator loss from both source and target batch
                    slight acc drop if use same batch size with source batch
                    '''
