...
def eval_with_threshold(model, dataloader, is_cda, unknown_class, threshold):
    ...
    with torch.no_grad():
        '''
        prediction: Find label using cosine similarity between test data and training data
        cosine similarity score becomes weight for threshold
        '''
        for test_batch in tqdm(dataloader, desc='testing '):
            test_batch = {k: v.cuda() for k, v in test_batch.items()}
            labels = test_batch['labels']
...
def main(args, save_config):
    ...
    if 'source_domain' in args.dataset:
        ...
        log_dir = f'{args.log.output_dir}/{args.dataset.name}/lacl/{source_domain}-{target_domain}/common-class-{args.dataset.num_common_class}/{args.train.seed}/{args.train.lr}'
    # clinc, massive, trec
    else:
        source_domain = None
        target_domain = None
        ...
        ## LOGGINGS ##
        log_dir = f'{args.log.output_dir}/{args.dataset.name}/lacl/{split}/common-class-{args.dataset.num_common_class}/{args.train.seed}/{args.train.lr}'
    print(f'{log_dir=}')
    # init logger
